...
function trp_developer_update(){
    // this is the URL our updater / license checker pings. This should be the URL of the site with EDD installed
    define('TRP_DEVELOPER_STORE_URL', 'https://translatepress.com');
    // the name of your product. This should match the download name in EDD exactly
    define('TRP_DEVELOPER_ITEM_NAME', 'TranslatePress Developer');
...
add_action( 'plugins_loaded', 'trp_developer_update', 0 );
/** End the update initialization here **/


if( !defined( 'TRANSLATE_PRESS' ) )
    define( 'TRANSLATE_PRESS', 'TranslatePress - Developer' );

register_activation_hook(__FILE__, 'trp_developer_activate');
function trp_developer_activate( $network_wide ) {
    if( !function_exists('is_plugin_active') )
        include_once( ABSPATH . '/wp-admin/includes/plugin.php' );

    if( is_plugin_active('translatepress-personal/index.php') || is_plugin_active('translatepress-business/index.php') ){
        set_transient( 'trp_deactivate_developer', true );
    }else{
        delete_transient( 'trp_deactivate_developer' );
    }
}


add_action('admin_notices', 'trp_developer_admin_notice');
function trp_developer_admin_notice(){
    $trp_deactivate_developer = get_transient( 'trp_deactivate_developer' );
    if( $trp_deactivate_developer ){

        $other_plugin_name = '';
        if( is_plugin_active('translatepress-personal/index.php') )
            $other_plugin_name = 'TranslatePress - Personal';
        else if( is_plugin_active('translatepress-business/index.php') )
...
add_action( 'admin_init', 'trp_developer_plugin_deactivate' );
function trp_developer_plugin_deactivate() {
