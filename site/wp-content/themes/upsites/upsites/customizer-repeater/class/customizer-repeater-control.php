...
class Customizer_Repeater extends WP_Customize_Control
{
	...
	public $customizer_repeater_image_mobile_control = false;
	...
	public function __construct($manager, $id, $args = array())
	{
		...
		if (!empty($args['customizer_repeater_image_mobile_control'])) {
			$this->customizer_repeater_image_mobile_control = $args['customizer_repeater_image_mobile_control'];
		}
		...
		if (file_exists(get_template_directory() . '/upsites/customizer-repeater/inc/icons.php')) {
			$this->customizer_icon_container =  'upsites/customizer-repeater/inc/icons';
		}
	...
	{
		wp_enqueue_style('font-awesome', get_template_directory_uri() . '/upsites/customizer-repeater/css/font-awesome.min.css', array(), CUSTOMIZER_REPEATER_VERSION);

		wp_enqueue_style('customizer-repeater-admin-stylesheet', get_template_directory_uri() . '/upsites/customizer-repeater/css/admin-style.css', array(), CUSTOMIZER_REPEATER_VERSION);

		wp_enqueue_style('wp-color-picker');

		wp_enqueue_script('customizer-repeater-script', get_template_directory_uri() . '/upsites/customizer-repeater/js/customizer_repeater.js', array('jquery', 'jquery-ui-draggable', 'wp-color-picker'), CUSTOMIZER_REPEATER_VERSION, true);

		wp_enqueue_script('customizer-repeater-fontawesome-iconpicker', get_template_directory_uri() . '/upsites/customizer-repeater/js/fontawesome-iconpicker.min.js', array('jquery'), CUSTOMIZER_REPEATER_VERSION, true);

		wp_enqueue_style('customizer-repeater-fontawesome-iconpicker-script', get_template_directory_uri() . '/upsites/customizer-repeater/css/fontawesome-iconpicker.min.css', array(), CUSTOMIZER_REPEATER_VERSION);
	}
	...
	private function iterate_array($array = array())
	{
		...
		if (!empty($array)) {
			...
			foreach ($array as $key => $icon) { ?>
				<div class="customizer-repeater-general-control-repeater-container customizer-repeater-draggable">
					<div class="customizer-repeater-customize-control-title">
						<?php
						echo esc_html($this->boxtitle) . ': <span class="count">' . $count++ . '</span>';
						?>
					</div>
					<div class="customizer-repeater-box-content-hidden">
						<?php
						$choice = $image_url = $image_mobile_url = $icon_value = $title = $subtitle = $text = $text2  = $link2 = $link = $shortcode = $repeater = $color = $color2 = '';
						if (!empty($icon->id)) {
							$id = $icon->id;
						}
						...
						if (!empty($icon->image_mobile_url)) {
							$image_mobile_url = $icon->image_mobile_url;
						}
						...
						if ($this->customizer_repeater_image_mobile_control == true) {
							$this->icon_type_choice($choice);
						}

						if ($this->customizer_repeater_image_mobile_control == true) {
							$this->image_mobile_control($image_mobile_url, $choice);
						}
			...
			<div class="customizer-repeater-general-control-repeater-container">
				<div class="customizer-repeater-customize-control-title">
					<?php echo esc_html($this->boxtitle) . ': <span class="count">' . $count++ . '</span>' ?>
				</div>
				<div class="customizer-repeater-box-content-hidden">
					<?php
					//if ($this->customizer_repeater_image_control == true && $this->customizer_repeater_icon_control == true) {
		...
		if (!empty($options['type'])) {
			switch ($options['type']) {
				...
				case 'color':
					...
					<span class="customize-control-title" <?php if (!empty($style_to_add)) {
																									echo 'style="' . esc_attr($style_to_add) . '"';
																								} ?>><?php echo esc_html($options['label']); ?></span>
					<div class="<?php echo esc_attr($options['class']); ?>" <?php if (!empty($style_to_add)) {
																																		echo 'style="' . esc_attr($style_to_add) . '"';
																																	} ?>>
	...
	{ ?>
		<div class="customizer-repeater-image-control" <?php if ($show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' || empty($show)) {
																											//echo 'style="display:none;"';
																										} ?>>
			<span class="customize-control-title">
	...
	private function image_mobile_control($value = '', $show = '')
	{ ?>
		<div class="customizer-repeater-image-control" <?php if ($show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' || empty($show)) {
																											//echo 'style="display:none;"';
																										} ?>>
			<span class="customize-control-title">
				<?php esc_html_e('Image mobile', 'your-textdomain') ?>
			</span>
			<input type="text" class="widefat custom-media-mobile-url" value="<?php echo esc_attr($value); ?>">
			<input type="button" class="button button-secondary customizer-repeater-custom-media-mobile-button" value="<?php esc_attr_e('Upload Image Mobile', 'your-textdomain'); ?>" />
		</div>
	<?php
	}
			...
			<div class="customizer-repeater-social-repeater">
				...
				foreach ($social_repeater as $social_icon) {
					...
					<div class="customizer-repeater-social-repeater-container">
						...
						<?php get_template_part($this->customizer_icon_container); ?>
						<input type="text" class="customizer-repeater-social-repeater-link" placeholder="<?php esc_attr_e('Link', 'your-textdomain'); ?>" value="<?php if (!empty($social_icon['link'])) {
																																																																												echo esc_url($social_icon['link']);
																																																																											} ?>">
						<input type="hidden" class="customizer-repeater-social-repeater-id" value="<?php if (!empty($social_icon['id'])) {
																																													echo esc_attr($social_icon['id']);
																																												} ?>">
						<button class="social-repeater-remove-social-item" style="<?php if ($show_del == 1) {
																																				echo "display:none";
